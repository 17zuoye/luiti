...
class CompressUtils:
    ...
    def unzip_with_upload(orig_filepath, hdfs_filepath, tmp_dir=None, tmp_name=None):
        ...
        while ( os.path.isdir(unzip_file) ):
            ...
            if len(next_dirs) >  1: raise ValueError("Should only one dir in a zip file!")
            if len(next_dirs) == 0: raise ValueError("Must always exists one file or one dir in a zip file!")
