...
IOError: [Errno 104] Connection reset by peer
    ...
    def run(cls, curr_task, date_value):
        ...
        curr_task_intance = curr_task(date_value=date_value)
        ordered_task_instances_list = SensorSchedule.read_all_required_tasks(curr_task_intance)

        # 3. execute tasks
        for t1 in ordered_task_instances_list:
            should_wait = SensorSchedule.is_external(t1.__class__)
            if should_wait:
                sleep_seconds = 0
                while True:
                    # 1. output arrives
    ...
    @staticmethod
    def read_all_required_tasks(curr_task_intance):
        """
        ...
        task_instances_dep_dict = func(dict(), curr_task_intance)
